    ...
    def get(self, request, *args, **kwargs):
        ...
        for materia in materias:
            materia_data = {
                ...
                'hora_inicio': materia.hora_inicio.strftime('%H:%M') if materia.hora_inicio else '',
                'hora_fin': materia.hora_fin.strftime('%H:%M') if materia.hora_fin else '',
                'salon': materia.salon,
            ...
            if materia.dias:
                try:
                    import json
                    dias_list = json.loads(materia.dias)
                    materia_data['dias'] = dias_list
                    materia_data['dias_formatted'] = ', '.join(dias_list)
                except:
                    dias_list = materia.dias.split(',') if materia.dias else []
                    materia_data['dias'] = dias_list
                    materia_data['dias_formatted'] = ', '.join(dias_list)
            else:
                materia_data['dias'] = []
                materia_data['dias_formatted'] = ''
            
            # Procesar profesor
            if materia.profesor and materia.profesor.user:
                materia_data['profesor_id'] = materia.profesor.id
                materia_data['profesor_nombre'] = f"{materia.profesor.user.first_name} {materia.profesor.user.last_name}"
                materia_data['profesor'] = materia_data['profesor_nombre']  # Para compatibilidad
            else:
                materia_data['profesor_id'] = None
                materia_data['profesor_nombre'] = 'No asignado'
                materia_data['profesor'] = 'No asignado'
...
# Agregar estas clases al final de materias.py

class TotalMateriasPorProgramaView(generics.CreateAPIView):
    permission_classes = (permissions.IsAuthenticated,)
    
    def get(self, request, *args, **kwargs):
        try:
            # Obtener total de materias por programa educativo
            programas = [
...
class DistribucionMateriasPorDiasView(generics.CreateAPIView):
    permission_classes = (permissions.IsAuthenticated,)
    
    def get(self, request, *args, **kwargs):
        try:
            # Distribución de materias por días de la semana
            dias_semana = ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes']
            distribucion = {}
            
            for dia in dias_semana:
                # Contar materias que tienen este día en su array de días
                count = 0
                ...
                for materia in materias:
                    if materia.dias:
                        try:
                            ...
                            if dia in materia.dias:
                                count += 1
                
                distribucion[dia] = count
            
            return Response(distribucion, 200)
        except Exception as e:
            return Response({"error": str(e)}, status=400)